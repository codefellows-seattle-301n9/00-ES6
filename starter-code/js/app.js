...
function displayPics(){
  ...
  // When we changed the 'rando' variable from 'var' to 'let', it made that variable only accesable inside that function, it is local only to that block of code.
  console.log(viewed)
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} had ${allProducts[i].votes} votes in ${allProducts[i].views} views`);
    }
  }
